...
def load_partition(trainset, testset, num_examples, idx, num_partitions = 5):
    ...
    train_partition = torch.utils.data.Subset(
        trainset, range((idx + settings.TRAIN_IDX - 1) * n_train, (idx + settings.TRAIN_IDX) * n_train)
    )
    test_partition = torch.utils.data.Subset(
        testset, range((idx + settings.TRAIN_IDX - 1) * n_test, (idx + settings.TRAIN_IDX) * n_test)
    )
